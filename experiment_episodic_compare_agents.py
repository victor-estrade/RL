...
def doExperiments(env,agents,n_learning_sessions,n_episodes,max_actions_per_episode,verbose=True):
    ...
    Returns:
        the mean learning curves for each experiments, it is a dictionary, where the key is the name of the agent, and the value are lists or arrays of returns per episode (averaged over n_learning_sessions).
    ...
    for agent_name, agent in agents.items(): 
        ...
        for i_learning_session in range(n_learning_sessions):
            ...
            print('env='+env_name+'   agent='+agent_name+'   learning session='+str(i_learning_session+1))
            
            all_rewards = doEpisodes(env,agent,n_episodes,max_actions,verbose)
...
if __name__ == '__main__':
    ...
    agents['Random'] = AgentRandom(n_states, n_actions)
    
    from agents.agent_first_action import AgentFirstAction
    agents['FirstAction'] = AgentFirstAction(n_states, n_actions)
    ...
    learning_curves = doExperiments(env,agents,n_learning_sessions,n_episodes,max_actions,verbose)
